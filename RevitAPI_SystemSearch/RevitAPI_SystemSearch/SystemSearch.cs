...
namespace RevitAPI_SystemSearch
{
    public class SystemSearch
    {
        ...
        public void RunStepThroughElements()
        {
            ...
            foreach (var pId in m_lSelectedElts)
            {
                ...
                if (pNext == null)
                {
                    ...
                    //Initial Element ^^^^^
                    //Do what you want with the Element here... you won't be seeing it again.
                    //Get data, etc.
                    m_lVistited.Add(pId);
                }
                else pPrev = pNext;
                ...
                {
                    pNext = GetNextConnectedElement(pConn, pPrev, ref m_lVistited);
                    //Do what you want with the Elements here...
                    //Get data, etc.
                    //And make sure we mark it as visited so we don't come back to it.
                    m_lVistited.Add(pNext.Id);                
                }
                //This will run through every selected Element.  
                //If for some reason, we missed something, we make it recursive
            }
            m_iLoops++;
            ...
            if ((m_lVistited.Count < m_lSelectedElts.Count) &&
                ...
                //so we give it a max of 20 tries to look at everything.
                RunStepThroughElements();
        }

        /// <summary>
        /// Return true if we want to look at the next Connector, and false if the current Connector is desired
        /// </summary>
        ...
        /// <summary>
        /// Gets the Element the Connector is connected to if it has not been visited before.
        /// Currently set to work with Pipe and FamilyInstance types - change types to accomodate
        /// </summary>
        ...
        /// <returns></returns>
        public Element GetNextConnectedElement(Connector pConn, Element pPrevElem, ref List<ElementId> lVisited)
        {
        ...
        {
            if (pElt is Pipe)
                return ((Pipe)pElt)?.ConnectorManager?.Connectors;
            if (pElt is FamilyInstance)
                return ((FamilyInstance)pElt)?.MEPModel?.ConnectorManager?.Connectors;
            //Add other statements to accomodate your needs based on different element types
            return null;
        }
